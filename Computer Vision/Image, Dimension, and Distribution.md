...
이미지의 색을 표현하기 위해서 color의 경우에는 RGB ⇒ 3차원이 필요하고, Gray color 의 경우에는 gray ⇒1차원이 필요하다.
...
그렇다면 색을 표혈할 수 있는 모든 경우의 수는?

RGB color의 경우에는 (R,G,B) ⇒ (255,255,255) 이므로 조합의 경우의 수를 구하면 $255^3$ 이 된다.

Gray color의 경우에는 (G) ⇒ (255) 가 된다.

위는 색을 표현하기 위한 차원을 알아봤고 이번에는 이미지를 표현하기 위한 차원을 알아보자.
